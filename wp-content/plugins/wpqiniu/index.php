...
Description: WordPress同步附件内容远程至七牛云对象存储中，实现网站数据与静态资源分离，提高网站加载速度。微信公众号：  <font color="red">站长事儿</font>
Version: 4.6
Author: 老部落
...
if (!class_exists('WPQiNiu')) {
    class WPQiNiu {
        ...
        private $version         = '4.4';                         // 插件数据版本， 每次修改应与上方的Version值相同
        private $setting_notices = [
                    'update_success' => 'WP七牛插件设置保存完毕',  // post数据保存成功时提示内容
                    'update_failed'  => 'WP七牛插件设置更新失败',  // 失败时提示
                ];
        private $image_display_default_value = 'imageView2/0/format/webp/q/75';  // 数据万象默认规则
        private $image_display_default_tab   = '?';                              // 万象规则url连字符

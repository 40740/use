...
if($_POST)
{
	...
	elseif(isset($_POST['paytype']) && $paytype==21 && plugin_check_epay())
	{
		$url=ERPHPDOWN_EPAY_URL."/epay.php?ice_money=".esc_sql($_POST['ice_money'])."&type=alipay";
	}elseif(isset($_POST['paytype']) && $paytype==22 && plugin_check_epay())
	{
		$url=ERPHPDOWN_EPAY_URL."/epay.php?ice_money=".esc_sql($_POST['ice_money'])."&type=wxpay";
	}
			...
			<tr>
				...
				<td>
					...
					<?php if(get_option('erphpdown_codepay_appid')){?> 
						...
						<?php if(!get_option('erphpdown_codepay_qqpay')){?><input type="radio" id="paytype15" class="paytype" name="paytype" value="15" onclick="checkCard()"/>QQ钱包&nbsp;<?php }?>
					<?php }?>
					<?php if(plugin_check_epay() && get_option('erphpdown_epay_id')){?>
						...
						<?php if(!get_option('erphpdown_epay_wxpay')){?><input type="radio" id="paytype22" class="paytype" name="paytype" value="22" onclick="checkCard()"/>微信<?php }?>
					<?php }?>
				</td>
			</tr>
		</table>

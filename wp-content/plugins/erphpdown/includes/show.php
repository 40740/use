...
function erphpdown_content_show($content){
	...
	if(!$down_box_hide){
		...
		if(is_singular()){
			...
			$down_info = null;$downMsgFree = '';$down_checkpan = '';$yituan = '';$down_tuan=0;$erphp_popdown='';$iframe='';$down_repeat=0;
			...
			if($start_down2){
								...
								if($memberDown==4 && $userType==FALSE){
									...
									if($userType && $memberDown > 1){
										if($memberDown==3 || $memberDown==4){
											...
											$content.='<a href='.constant("erphpdown").'download.php?url='.$down_info->ice_url.$iframe.' class="erphpdown-down'.$erphp_popdown.'" target="_blank">立即下载</a>';
										}
									}else{
										if($down_info){
											$content.='<a href='.constant("erphpdown").'download.php?url='.$down_info->ice_url.$iframe.' class="erphpdown-down'.$erphp_popdown.'" target="_blank">已购买，立即下载</a>';
				...
				if(is_user_logged_in()){
					...
					if( ($userType && ($memberDown==3 || $memberDown==4)) || $down_info || (($memberDown==6 || $memberDown==8) && $userType >= 9) || (($memberDown==7 || $memberDown==9 || $memberDown==13 || $memberDown==14) && $userType == 10) || (!$price && $memberDown!=4 && $memberDown!=8 && $memberDown!=9)){
						return $content;
					}else{
				...
				if(is_user_logged_in()){
					...
					if( ($userType && ($memberDown==3 || $memberDown==4)) || ($down_info && $down_info->ice_price > 0) || (($memberDown==6 || $memberDown==8) && $userType >= 9) || (($memberDown==7 || $memberDown==9 || $memberDown==13 || $memberDown==14) && $userType == 10) || (!$price && $memberDown!=4 && $memberDown!=8 && $memberDown!=9)){

		...
		if(!$money_info->ice_success){
			...
			if($money_info->ice_post_id){
                    ...
                    if(erphpSetUserMoneyXiaoFeiByUid($price,$money_info->ice_user_id))
                    {
                        ...
                        if($result)
                        {
							...
							if(get_option('erphp_remind')){
								wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】订单提醒 - '.$subject, '用户'.get_user_by('id',$money_info->ice_user_id)->user_login.'消费'.$price.get_option('ice_name_alipay').'购买了'.$subject.get_permalink($postid));
							}
                        } 
                    }
                }
				...
				if(get_option('erphp_remind_recharge')){
					wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】充值提醒 - '.$total_fee_old.'元', '用户'.get_user_by('id',$money_info->ice_user_id)->user_login.'刚刚充值了'.$total_fee_old.'元。');
				}
			}
		}
	}
...
function erphpGetUserOkMoney()
{
	...
	if($user_info)
	{
		...
		return $userMoney==false ?0:sprintf("%.2f",($userMoney->ice_have_money - $userMoney->ice_get_money));
	}
	return 0;
}
...
function erphpdown_dashboard_widget_function() {
	...
	if(current_user_can('administrator')){
		...
		echo '</ul><p style="opacity:.7;font-size:12px;">收入 = 充值额 + 免登录销售额</p></div><div style="border-top:1px solid #eee;padding: 8px 12px 4px;margin:0 -12px"><i class="dashicons dashicons-external"></i> 强烈推荐资源下载主题<a href="https://www.mobantu.com/7191.html" target="_blank">Modown</a>。</div>';
	}else{
		$user_info=wp_get_current_user();
		$userMoney=$wpdb->get_row("select * from ".$wpdb->iceinfo." where ice_user_id=".$user_info->ID);
		if(!$userMoney){
		...
		echo '</ul><div style="border-top:1px solid #eee;padding: 8px 12px 4px;margin:0 -12px"><i class="dashicons dashicons-external"></i> 强烈推荐资源下载主题<a href="https://www.mobantu.com/7191.html" target="_blank">Modown</a>。</div>';
	}
}
...
function erphpdown_shortcode_box(){
	...
	$userType=getUsreMemberType();
	$down_info = null;$downMsgFree = '';$yituan = '';$down_tuan=0;$iframe='';$erphp_popdown='';$down_checkpan = '';$down_repeat=0;
		...
		if($url){
			if(function_exists('epd_check_pan_callback')){
				if(strpos($url,'pan.baidu.com') !== false){
					$down_checkpan = '<a class="erphpdown-buy erphpdown-checkpan2" href="javascript:;" data-id="'.get_the_ID().'" data-post="'.get_the_ID().'">网盘链接，点击检测有效后购买</a>';
				}
			}
			...
			if($wppay->isWppayPaid() || !$price || ($memberDown == 3 && $userType)){
				...
				if($memberDown == 3){
					if($down_checkpan) $content .= $tname.'价格<span class="erphpdown-price">'.$price.'</span>元'.$down_checkpan.'&nbsp;&nbsp;<b>或</b>&nbsp;&nbsp;升级'.$erphp_vip_name.'后免费<a href="'.$erphp_url_front_vip2.'" target="_blank" class="erphpdown-vip'.(is_user_logged_in()?'':' erphp-login-must').'">升级'.$erphp_vip_name.'</a>';
					else $content .= $tname.'价格<span class="erphpdown-price">'.$price.'</span>元<a href="javascript:;" class="erphp-wppay-loader erphpdown-buy" data-post="'.get_the_ID().'">立即支付</a>&nbsp;&nbsp;<b>或</b>&nbsp;&nbsp;升级'.$erphp_vip_name.'后免费<a href="'.$erphp_url_front_vip2.'" target="_blank" class="erphpdown-vip'.(is_user_logged_in()?'':' erphp-login-must').'">升级'.$erphp_vip_name.'</a>';
				}else{
					if($down_checkpan) $content .= $tname.'价格<span class="erphpdown-price">'.$price.'</span>元'.$down_checkpan;
					else $content .= $tname.'价格<span class="erphpdown-price">'.$price.'</span>元<a href="javascript:;" class="erphp-wppay-loader erphpdown-buy" data-post="'.get_the_ID().'">立即支付</a>';	
				}
			}
	    					...
	    					if(is_user_logged_in()){
								...
								if($price || $indexMemberDown == 4 || $indexMemberDown == 8 || $indexMemberDown == 9){
									...
									if($indexMemberDown==4 && $userType==FALSE){
										...
										if($down_info){
											$content.='<a href='.constant("erphpdown").'download.php?url='.$down_info->ice_url.$iframe.' class="erphpdown-down'.$erphp_popdown.'" target="_blank">已购买，立即下载</a>';
										}elseif($userType){
										...
										if($userType && $indexMemberDown > 1){
											...
											if($down_info){
												$content.='<a href="'.constant("erphpdown").'download.php?postid='.get_the_ID().'&index='.$index.$iframe.'" class="erphpdown-down'.$erphp_popdown.'" target="_blank">已购买，立即下载</a>';
											}else{
												$content.='<a class="erphpdown-iframe erphpdown-buy" href="'.constant("erphpdown").'buy.php?postid='.get_the_ID().'&index='.$index.'" target="_blank">立即购买</a>';
											}
										}
					...
					if($price || $memberDown == 4 || $memberDown == 8 || $memberDown == 9){
						...
						if($memberDown==4 && $userType==FALSE){
							...
							if($userType && $memberDown > 1){
								...
								if($down_info){
									$content.='<a href='.constant("erphpdown").'download.php?postid='.get_the_ID().$iframe.' class="erphpdown-down'.$erphp_popdown.'" target="_blank">已购买，立即下载</a>';
								}else{
									$content.='<a class="erphpdown-iframe erphpdown-buy" href='.constant("erphpdown").'buy.php?postid='.get_the_ID().' target="_blank">立即购买</a>';
								}
							}

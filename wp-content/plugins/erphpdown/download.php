...
if ($postid){
	...
	if(!$hasdown_info && is_user_logged_in()){
		if(!$price && $memberDown != 4 && $memberDown != 8 && $memberDown != 9){
			...
			if($memberDown == 3 || $memberDown == 4 || $memberDown == 6 || $memberDown == 7 || $memberDown == 8 || $memberDown == 9 || $memberDown == 13 || $memberDown == 14){
...
if($key){
	...
	foreach ($downList as $k=>$v){
		...
		if($filepath){
			...
			if(strpos($filepath,',')){
				...
				if($arrlength == 1){
					$downMsg.="<p><span class='tit'>文件".($k+1)."地址</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link'>点击下载</a></p>";
				}elseif($arrlength == 2){
					$downMsg.="<p><span class='tit'>".$filearr[0]."</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link'>点击下载</a></p>";
				}elseif($arrlength == 3){
					$filearr2 = str_replace('：', ': ', $filearr[2]);
					$filearr3 = explode(':',$filearr2);
					$arrlength = count($filearr3);
					if($arrlength >= 2){
						$downMsg.="<p><span class='tit'>".$filearr[0]."</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link erphpdown-down-btn' data-clipboard-text='".trim($filearr3[1])."'>点击下载</a>".$filearr3[0].": <span class='erphpdown-code'>".trim($filearr3[1])."</span><a class='erphpdown-copy' data-clipboard-text='".trim($filearr3[1])."' href='javascript:;'>复制</a></p>";
					}else{
						$downMsg.="<p><span class='tit'>".$filearr[0]."</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link erphpdown-down-btn' data-clipboard-text='".$filearr2."'>点击下载</a><span class='erphpdown-code'>".$filearr2."</span><a class='erphpdown-copy' data-clipboard-text='".$filearr2."' href='javascript:;'>复制</a></p>";
					}
				}
				...
				if($arrlength == 1){
					...
					$downMsg.="<p><span class='tit'>".$filearr2[0]."</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link erphpdown-down-btn' data-clipboard-text='".trim($filearr3[1])."'>点击下载</a>提取码: <span class='erphpdown-code'>".trim($filearr3[1])."</span><a class='erphpdown-copy' data-clipboard-text='".trim($filearr3[1])."' href='javascript:;'>复制</a></p>";
				}
			}elseif(strpos($filepath,' ') && $erphp_blank_domain_is){
				$filearr = explode(' ',$filepath);
				$arrlength = count($filearr);
				if($arrlength == 1){
					...
					$downMsg.="<p><span class='tit'>".str_replace(':', '', $filearr[0])."</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link erphpdown-down-btn' data-clipboard-text='".$filearr[3]."' >点击下载</a>".$filearr[2].' <span class="erphpdown-code">'.$filearr[3]."</span><a class='erphpdown-copy' data-clipboard-text='".$filearr[3]."' href='javascript:;'>复制</a></p>";
				}
			}else{
				$downMsg.="<p><span class='tit'>文件".($k+1)."地址</span><a href='download.php?postid=".$postid."&key=".($k+1)."&index=".$index."' target='_blank' class='link'>点击下载</a></p>";
			}
	...
	if($hiddens){
		...
		if($erphp_hide_style){
			$downMsg .='<div class="title hidden-title"><span>隐藏信息</span></div><div class="hidden-content">';
			$hiddens_arr1 = explode('，', $hiddens);
			foreach ($hiddens_arr1 as $hiddens1) {
				$hiddens_arr2 = explode('：', $hiddens1);
				if(count($hiddens_arr2) > 1){
					$downMsg .= '<div>'.$hiddens_arr2[0].'：<span class="erphpdown-code">'.trim($hiddens_arr2[1]).'</span><a class="erphpdown-copy" data-clipboard-text="'.trim($hiddens_arr2[1]).'" href="javascript:;" style="margin-left:10px;">复制</a></div>';
				}else{
					$downMsg .= '<div><span class="erphpdown-code">'.trim($hiddens1).'</span><a class="erphpdown-copy" data-clipboard-text="'.trim($hiddens1).'" href="javascript:;" style="margin-left:10px;">复制</a></div>';
				}
			}
			$downMsg .='</div>';
		}else{
			$downMsg .='<div class="title"><span>隐藏信息</span></div><div class="hidden-content" style="margin-top:10px;">'.$hiddens.'<a class="erphpdown-copy" data-clipboard-text="'.$hiddens.'" href="javascript:;" style="margin-left:10px;">复制</a></div>';
		}
	}

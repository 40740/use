...
function ouath_redirect($userid,$type,$avatar,$re_url){
    ...
    // 搜库资源网 soku.cc   如果是微信登录，分别记录获取储存的头像
    if($type == 'weixin'){
        if(zrz_is_weixin()){
...
function get_token($code,$type){
    ...
    if($type == 'qq'){
        $url = "https:// 搜库资源网 soku.cc   graph.qq.com/oauth2.0/token";
        $client_id = ZRZ_QQ_APPID;
        $client_secret = ZRZ_QQ_APPSECRET;
    }elseif($type == 'weibo'){
        $url = "https:// 搜库资源网 soku.cc   api.weibo.com/oauth2/access_token";
        $client_id = ZRZ_WEIBO_APPID;
        $client_secret = ZRZ_WEIBO_APPSECRET;
    }elseif($type = 'weixin'){
        $url = 'https:// 搜库资源网 soku.cc   api.weixin.qq.com/sns/oauth2/access_token';
    }
...
function all_oauth($type,$code,$re_url){
    ...
    if($type == 'qq'){
        $graph_url = "https:// 搜库资源网 soku.cc   graph.qq.com/oauth2.0/me?access_token=" . $access_token;
        ...
        $get_user_info_url = "https:// 搜库资源网 soku.cc   graph.qq.com/user/get_user_info?" . "access_token=" . $access_token . "&oauth_consumer_key=" . ZRZ_QQ_APPID . "&openid=" . $qq_openid . "&format=json";
        $uid = $qq_openid;
        $username = 'nickname';
        ...
        $uid = $output['uid'];
        $get_user_info_url = "https:// 搜库资源网 soku.cc   api.weibo.com/2/users/show.json?uid=".$uid."&access_token=".$access_token;
        $username = 'screen_name';
        $avatar = 'avatar_large';
    }elseif($type == 'weixin'){

        $get_user_info_url = 'https:// 搜库资源网 soku.cc   api.weixin.qq.com/sns/userinfo?access_token=' . $output['access_token'] . '&openid=' . $output['openid'];
        $username = 'nickname';
        $avatar = 'headimgurl';
        $uid = $output['openid'];
        $access_token = $output['access_token'];
...
function save_login($type,$user_info,$re_url){
    ...
    if(is_user_logged_in()){
        ...
        if(is_wp_error($user) || empty($user)){
            if(!zrz_get_social_settings('open_regeister')){
                die('本站已关闭注册');
            }

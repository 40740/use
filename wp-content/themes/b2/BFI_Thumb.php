 ...
 * along with this program.  If not, see <http:// 搜库资源网 soku.cc   www.gnu.org/licenses/>.
 */

/* 搜库资源网 soku.cc* Uses WP's Image Editor Class to resize and filter images
 *
...
if ( ! defined( 'ABSPATH' ) ) exit; // 搜库资源网 soku.cc    Exit if accessed directly
...
// 搜库资源网 soku.cc    Instead of using the default image editors, use our extended ones
if ( ! function_exists( 'bfi_wp_image_editor' ) ) {

	function bfi_wp_image_editor( $editorArray ) {
	    // 搜库资源网 soku.cc    Make sure that we use the latest versions
	    return array(
...
if ( ! class_exists( 'BFI_Image_Editor_Imagick_1_3' ) ) {
	...
	class BFI_Image_Editor_Imagick_1_3 extends WP_Image_Editor_Imagick {
	    ...
	    /* 搜库资源网 soku.cc* Changes the opacity of the image
	     *
	     * @supports 3.5.1
	     * @access public
	     *
	    ...
	    public function opacity( $opacity ) {
	        ...
	        try {
	            // 搜库资源网 soku.cc    From: http:// 搜库资源网 soku.cc   stackoverflow.com/questions/3538851/php-imagick-setimageopacity-destroys-transparency-and-does-nothing
	            // 搜库资源网 soku.cc    preserves transparency
	            // 搜库资源网 soku.cc   $this->image->setImageOpacity($opacity);
	    ...
	    /* 搜库资源网 soku.cc* Tints the image a different color
	     *
	     * @supports 3.5.1
	     * @access public
	     *
	    ...
	    /* 搜库资源网 soku.cc* Makes the image grayscale
	     *
	     * @supports 3.5.1
	     * @access public
	     *
	    ...
	    /* 搜库资源网 soku.cc* Negates the image
	     *
	     * @supports 3.5.1
	     * @access public
	     *
...
if ( ! class_exists( 'BFI_Image_Editor_GD_1_3' ) ) {
	...
	class BFI_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
	    ...
	    /* 搜库资源网 soku.cc* Rotates current image counter-clockwise by $angle.
	    ...
	    public function rotate( $angle ) {
	        if ( function_exists('imagerotate') ) {
	            ...
	            // 搜库资源网 soku.cc    Add alpha blending
	    ...
	    public function opacity( $opacity ) {
	        ...
	        if ( is_resource( $filtered ) ) {
	            // 搜库资源网 soku.cc    imagedestroy($this->image);
	            $this->image = $filtered;
	            return true;
	        }
	    ...
	    // 搜库资源网 soku.cc    from: http:// 搜库资源网 soku.cc   php.net/manual/en/function.imagefilter.php
	    // 搜库资源网 soku.cc    params: image resource id, opacity (eg. 0.0-1.0)
	    protected function _opacity( $image, $opacity ) {
	        ...
	        // 搜库资源网 soku.cc    get image width and height
	        $w = imagesx( $image );
	        $h = imagesy( $image );

	        // 搜库资源网 soku.cc    turn alpha blending off
	        imagealphablending( $image, false );

	        // 搜库资源网 soku.cc    find the most opaque pixel in the image (the one with the smallest alpha value)
	        $minalpha = 127;
	        for ( $x = 0; $x < $w; $x++ ) {
	            for ( $y = 0; $y < $h; $y++ ) {
	        ...
	        // 搜库资源网 soku.cc    loop through image pixels and modify alpha for each
	        for ( $x = 0; $x < $w; $x++ ) {
	            for ( $y = 0; $y < $h; $y++ ) {

	                // 搜库资源网 soku.cc    get current alpha value (represents the TANSPARENCY!)
	                $colorxy = imagecolorat( $image, $x, $y );
	                $alpha = ( $colorxy >> 24 ) & 0xFF;

	                // 搜库资源网 soku.cc    calculate new alpha
	                if ( $minalpha !== 127 ) {
	                ...
	                // 搜库资源网 soku.cc    get the color index with new alpha
	                $alphacolorxy = imagecolorallocatealpha( $image, ( $colorxy >> 16 ) & 0xFF, ( $colorxy >> 8 ) & 0xFF, $colorxy & 0xFF, $alpha );

					// 搜库资源网 soku.cc    set pixel with the new color + opacity
...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
	...
	class BFI_Thumb_1_3 {
	    ...
	    /* 搜库资源网 soku.cc* Uses WP's Image Editor Class to resize and filter images
	     * Inspired by: https:// 搜库资源网 soku.cc   github.com/sy4mil/Aqua-Resizer/blob/master/aq_resizer.php
	     *
	     * @param $url string the local image URL to manipulate
	     * @param $params array the options to perform on the image. Keys and values supported:
	     *          'width' int pixels
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
	        ...
	        // 搜库资源网 soku.cc   define upload path & dir
	        ...
	        // 搜库资源网 soku.cc    find the path of the image. Perform 2 checks:
	        // 搜库资源网 soku.cc    #1 check if the image is in the uploads folder
	        if ( strpos( $url, $upload_url ) !== false ) {
	            $rel_path = str_replace( $upload_url, '', $url );
	            $img_path = $upload_dir . $rel_path;

	        // 搜库资源网 soku.cc    #2 check if the image is in the current theme folder
	        } else if ( strpos( $url, $theme_url ) !== false ) {
	            $rel_path = str_replace( $theme_url, '', $url );
	            $img_path = $theme_dir . $rel_path;
	        }

	        // 搜库资源网 soku.cc    Fail if we can't find the image in our WP local directory
	        if ( empty( $img_path ) ) {
	            return $url;
	        }

	        // 搜库资源网 soku.cc    check if img path exists, and is an image indeed
	        if( ! @file_exists( $img_path ) || ! getimagesize( $img_path ) ) {
	            return $url;
	        }

	        // 搜库资源网 soku.cc    This is the filename
	        $basename = basename( $img_path );

	        // 搜库资源网 soku.cc   get image info
	        $info = pathinfo( $img_path );
	        $ext = $info['extension'];
	        list( $orig_w, $orig_h ) = getimagesize( $img_path );

	        // 搜库资源网 soku.cc    support percentage dimensions. compute percentage based on
	        // 搜库资源网 soku.cc    the original dimensions
	        if ( isset( $width ) ) {
	        ...
	        // 搜库资源网 soku.cc    The only purpose of this is to determine the final width and height
	        // 搜库资源网 soku.cc    without performing any actual image manipulation, which will be used
	        // 搜库资源网 soku.cc    to check whether a resize was previously done.
	        if ( isset( $width ) && $crop_only === false ) {
	            // 搜库资源网 soku.cc   get image size after cropping
	            $dims = image_resize_dimensions( $orig_w, $orig_h, $width, isset( $height ) ? $height : null, isset( $crop ) ? $crop : false );
	            $dst_w = $dims[4];
	            $dst_h = $dims[5];

	        } else if ( $crop_only === true ) {
	            // 搜库资源网 soku.cc    we don't want a resize,
	            // 搜库资源网 soku.cc    but only a crop in the image

	            // 搜库资源网 soku.cc    get x position to start croping
	            $src_x = ( isset( $crop_x ) ) ? $crop_x : 0;

	            // 搜库资源网 soku.cc    get y position to start croping
	            $src_y = ( isset( $crop_y ) ) ? $crop_y : 0;

	            // 搜库资源网 soku.cc    width of the crop
	            if ( isset( $crop_width ) ) {
	                $src_w = $crop_width;
	            } else if ( isset( $width ) ) {
	                $src_w = $width;
	            ...
	            // 搜库资源网 soku.cc    height of the crop
	            ...
	            // 搜库资源网 soku.cc    set the width resize with the crop
	            ...
	            // 搜库资源网 soku.cc    set the height resize with the crop
	            ...
	            // 搜库资源网 soku.cc    allow percentages
	            if ( isset( $dst_w ) ) {
	            ...
	            // 搜库资源网 soku.cc    Make the pos x and pos y work with percentages
	            ...
	            // 搜库资源网 soku.cc    allow center to position crop start
	            if ( $src_x === 'center' ) {
	        ...
	        // 搜库资源网 soku.cc    create the suffix for the saved file
	        // 搜库资源网 soku.cc    we can use this to check whether we need to create a new file or just use an existing one.
	        ...
	        // 搜库资源网 soku.cc    use this to check if cropped image already exists, so we can return that instead
	        $dst_rel_path = str_replace( '.' . $ext, '', basename( $img_path ) );

	        // 搜库资源网 soku.cc    If opacity is set, change the image type to png
	        if ( isset( $opacity ) ) {
	            $ext = 'png';
	        }


	        // 搜库资源网 soku.cc    Create the upload subdirectory, this is where
	        // 搜库资源网 soku.cc    we store all our generated images
	        ...
	        // 搜库资源网 soku.cc    desination paths and urls
	        $destfilename = "{$upload_dir}/{$dst_rel_path}-{$suffix}.{$ext}";

			// 搜库资源网 soku.cc    The urls generated have lower case extensions regardless of the original case
			$ext = strtolower( $ext );
	        $img_url = "{$upload_url}/{$dst_rel_path}-{$suffix}.{$ext}";

	        // 搜库资源网 soku.cc    if file exists, just return it
	        if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
	        } else {
	            // 搜库资源网 soku.cc    perform resizing and other filters
				...
				// 搜库资源网 soku.cc    set the image quality (1-100) to save this image at
				if ( isset( $quality ) && $quality > 0 && $quality <= 100 && $ext != 'png' ) {
					$editor->set_quality( $quality );
				}

	            // 搜库资源网 soku.cc    save our new image
	            $mime_type = isset( $opacity ) ? 'image/png' : null;
	            $resized_file = $editor->save( $destfilename, $mime_type );
	        }

	        // 搜库资源网 soku.cc   return the output
	        if ( $single ) {
	            $image = $img_url;
	        } else {
	            // 搜库资源网 soku.cc   array return
	            $image = array (
	                0 => $img_url,
	    ...
	    /* 搜库资源网 soku.cc* Shortens a number into a base 36 string
	     *
...
// 搜库资源网 soku.cc    don't use the default resizer since we want to allow resizing to larger sizes (than the original one)
// 搜库资源网 soku.cc    Parts are copied from media.php
// 搜库资源网 soku.cc    Crop is always applied (just like timthumb)
// 搜库资源网 soku.cc    Don't use this inside the admin since sometimes images in the media library get resized
if ( ! is_admin() ) {
...
if ( ! function_exists( 'bfi_image_resize_dimensions' ) ) {
	function bfi_image_resize_dimensions( $payload, $orig_w, $orig_h, $dest_w, $dest_h, $crop = false ) {
		...
		// 搜库资源网 soku.cc    Safe guard against super large or zero images which might cause 500 errors
		if ( $new_w > 5000 || $new_h > 5000 || $new_w <= 0 || $new_h <= 0 ) {
			return null;
		}

	    // 搜库资源网 soku.cc    the return array matches the parameters to imagecopyresampled()
	    // 搜库资源网 soku.cc    int dst_x, int dst_y, int src_x, int src_y, int dst_w, int dst_h, int src_w, int src_h
	    return array( 0, 0, (int) $s_x, (int) $s_y, (int) $new_w, (int) $new_h, (int) $crop_w, (int) $crop_h );
	}
}


// 搜库资源网 soku.cc    This function allows us to latch on WP image functions such as
// 搜库资源网 soku.cc    the_post_thumbnail, get_image_tag and wp_get_attachment_image_src
// 搜库资源网 soku.cc    so that you won't have to use the function bfi_thumb in order to do resizing.
// 搜库资源网 soku.cc    To make this work, in the WP image functions, when specifying an
// 搜库资源网 soku.cc    array for the image dimensions, add a 'bfi_thumb' => true to
// 搜库资源网 soku.cc    the array, then add your normal $params arguments.
// 搜库资源网 soku.cc   
// 搜库资源网 soku.cc    e.g. the_post_thumbnail( array( 1024, 400, 'bfi_thumb' => true, 'grayscale' => true ) );

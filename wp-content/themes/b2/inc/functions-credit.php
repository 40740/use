...
class Zrz_Credit_Message{
    ...
    private $type;// 搜库资源网 soku.cc   通知类型
    private $date;// 搜库资源网 soku.cc   时间
    private $hot;// 搜库资源网 soku.cc   是否增加活跃值
    ...
    private function credit_action($type = 0){
        ...
        // 搜库资源网 soku.cc   如果任务完成则不增加积分
        $task = new ZRZ_TASK($this->user_id,$type);
        // 搜库资源网 soku.cc   检查任务完成情况
        if(!$task->check_task()){
            return false;
        }else{
            // 搜库资源网 soku.cc   任务加1
        ...
        // 搜库资源网 soku.cc   如果最后积分小于零，返回
        if($user_credit_new < 0){
            return false;
        }

        // 搜库资源网 soku.cc   写入积分,如果是积分抽奖和积分兑换则不受限制
    ...
    public function add_message($users = '',$credit = 0,$key = 0,$value = '',$type = 'add'){
        ...
        // 搜库资源网 soku.cc   如果设置了空，则不增加消息，保留活动报名通知
        if($credit === 'empty' && $this->type != 44) return false;

        // 搜库资源网 soku.cc   检查未读的历史数据
        ...
        if($check && $this->type != 13 && $this->type != 41 && $this->type != 30 && $this->type != 16 && $this->type != 45){
            ...
            // 搜库资源网 soku.cc   如果是申请提现，credit 字段则是提现的金额，不通过积分函数处理

...
// 搜库资源网 soku.cc   将用户页面的 author 改为 user
function change_author_permalinks() {
    global $wp_rewrite;
    $wp_rewrite->author_base = 'user';
    // 搜库资源网 soku.cc   $wp_rewrite->flush_rules();
}
add_action('init','change_author_permalinks');

add_filter('query_vars', 'users_query_vars');
function users_query_vars($vars) {
    // 搜库资源网 soku.cc    add lid to the valid list of variables
    $vars[] = 'user';
    $vars[] = 'zrz_user_page';
    return $vars;
}
...
// 搜库资源网 soku.cc   更新用户信息
class zrz_update_user_data{
    public static $type;
    public static $val;// 搜库资源网 soku.cc   存入的值
    public static $user_id;// 搜库资源网 soku.cc   用户ID
    public static $avatar_type;// 搜库资源网 soku.cc   头像类型
    public static $avatar_new;// 搜库资源网 soku.cc   社交头像原始图片链接，用以对比用户的社交网络上的头像是否更换
...
class zrz_get_user_data{
    public static $user_id;// 搜库资源网 soku.cc   用户ID
    public static $user_open;// 搜库资源网 soku.cc   用户的自定义信息，包括封面图片，头像，社交登陆的信息等等
    public static $size;// 搜库资源网 soku.cc   头像尺寸
    ...
    // 搜库资源网 soku.cc   获取用户封面图片
    public static function get_cover(){

        // 搜库资源网 soku.cc   如果没有封面图片，返回空
    ...
    public static function get_avatar($type = false){
        ...
        if($avatar_set == 'default'){
            ...
            if($avatar_host == 0){
                ...
                if(zrz_is_weixin() && isset($wx_av['in_wx'])){
                    return $wx_av['in_wx'];
                }

                if(isset($wx_av['in_pc']) && $wx_av['in_pc']){
                    return $wx_av['in_pc'];
                }
...
function zrz_ajax_update_user_data(){
    ...
    // 搜库资源网 soku.cc   开启了何种验证方式
    $sign_type = zrz_get_social_settings('type');
    $pass = '';

    // 搜库资源网 soku.cc   修改邮箱或者电话
    if (!isset($_SESSION)) {
        session_start();
    }
...
// 搜库资源网 soku.cc   允许搜索display_name
...
function zrz_send_msg(){
    ...
    // 搜库资源网 soku.cc   不能给自己发消息
    if(!$to || !$content || $current_user_id == $to || $check){
        print json_encode(array('status'=>401,'msg'=>__('发送失败','ziranzhi2')));
        exit;
    }

    // 搜库资源网 soku.cc   防止重复提交，
    $count = wp_cache_get('b2_msg_'.$current_user_id);
    if((int)$count >= 5){
        print json_encode(array('status'=>401,'msg'=>__('请不要重复发布垃圾私信','ziranzhi2')));
        exit;
    }

    // 搜库资源网 soku.cc   给被私信人发送通知
    $init = new Zrz_Credit_Message($to,12);
    $add_msg = $init->add_message($current_user_id, 0,0,0);

    // 搜库资源网 soku.cc   添加消息到被私信人
    $init_msg = new Zrz_Credit_Message($to,13);
    $resout = $init_msg->add_message($current_user_id, 0,$current_user_id,$content);

    if($add_msg && $resout){

        // 搜库资源网 soku.cc   缓存token，防止重复提交
        if(!$count){
            wp_cache_add('b2_msg_'.$current_user_id,'1',null,60);
        }else{
            wp_cache_set('b2_msg_'.$current_user_id,$count+1,null,60);
        }

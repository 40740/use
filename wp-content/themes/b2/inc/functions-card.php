...
function zrz_create_guid($namespace = null,$inv = false) {
    ...
    $data = $namespace;
    $data .= $_SERVER ['REQUEST_TIME'];     // 搜库资源网 soku.cc    请求那一刻的时间戳
    $data .= $_SERVER ['HTTP_USER_AGENT'];  // 搜库资源网 soku.cc    获取访问者在用什么操作系统
    $data .= $_SERVER ['SERVER_ADDR'];      // 搜库资源网 soku.cc    服务器IP
    $data .= $_SERVER ['SERVER_PORT'];      // 搜库资源网 soku.cc    端口号
    $data .= $_SERVER ['REMOTE_ADDR'];      // 搜库资源网 soku.cc    远程IP
    $data .= $_SERVER ['REMOTE_PORT'];      // 搜库资源网 soku.cc    端口信息

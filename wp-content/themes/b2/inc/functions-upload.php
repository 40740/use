...
class zrz_media{
    ...
    public static $mime;// 搜库资源网 soku.cc   文件格式
    public static $type;// 搜库资源网 soku.cc   文件是什么类型？头像还是文章图片..
    public static $file;// 搜库资源网 soku.cc   文件
    public static $user_id;// 搜库资源网 soku.cc   当前用户ID
    public static $is_letter_avatar;// 搜库资源网 soku.cc   如果非空，则为用户名
    public static $subdir;// 搜库资源网 soku.cc   日期目录
    public static $cover_y;// 搜库资源网 soku.cc   用户封面图片裁剪的坐标
    public static $key;// 搜库资源网 soku.cc   文件名
    public static $keyi;// 搜库资源网 soku.cc   文件名，没有后缀
    ...
    public function __construct($file,$type = '',$user_id = 0,$is_letter_avatar = '',$cover_y = 0){
        ...
        // 搜库资源网 soku.cc   如果没有指定用户ID，则使用当前登陆的用户ID
        if(!$user_id){
        ...
        // 搜库资源网 soku.cc   是否是头像上传
        if($is_letter_avatar){
            self::$is_letter_avatar = $is_letter_avatar;
        }

        // 搜库资源网 soku.cc   获取文件的扩展名
        $path = self::$file['name'];
        self::$mime = pathinfo($path, PATHINFO_EXTENSION);

        // 搜库资源网 soku.cc   用户封面图片裁剪的Y坐标
        if($cover_y)
        self::$cover_y = $cover_y;

        // 搜库资源网 soku.cc   路径
        $path = wp_upload_dir();
        // 搜库资源网 soku.cc   $upload_dir = zrz_upload_dir($path);
    ...
    public static function get_media_key(){
        ...
        $ext = '.'.self::$mime;
        self::$keyi = $key;
        // 搜库资源网 soku.cc   如果是图片，为图片添加后缀，已示区分
        switch (self::$type) {
            case 'cover':
...
function zrz_upload_dir($param){
    ...
    if($yun_setting['host'] && $stream){
        if(isset($yun_setting['path']) && !empty($yun_setting['path'])){
            $param['basedir'] = $stream.':// 搜库资源网 soku.cc   '.trim($yun_setting['bucket'], '/').'/'.trim($yun_setting['path'],'/');
            ...
            $param['basedir'] = $stream.':// 搜库资源网 soku.cc   '.trim($yun_setting['bucket'], '/');
            $param['path'] = $param['basedir'] .'/'. trim($param['subdir'],'/');

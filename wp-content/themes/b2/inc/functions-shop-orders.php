...
/* 搜库资源网 soku.cc
* 商城订单项
* $order_type // 搜库资源网 soku.cc   订单类型
* c : 抽奖 ，d : 兑换 ，g : 购买 ，w : 文章内购 ，ds : 打赏 ，cz : 充值 ，vip : VIP购买 ,cg : 积分购买
*
* $order_commodity // 搜库资源网 soku.cc   商品类型
* 0 : 虚拟物品 ，1 : 实物
*
* $order_state // 搜库资源网 soku.cc   订单状态
...
class Zrz_order_Message{
    ...
    private $user_id;// 搜库资源网 soku.cc   用户ID
    private $post_id;// 搜库资源网 soku.cc   用户ID
    private $type;// 搜库资源网 soku.cc   订单类型
    private $order_commodity;// 搜库资源网 soku.cc   商品类型
    private $date;// 搜库资源网 soku.cc   时间
    private $order_id;// 搜库资源网 soku.cc   订单号
    private $order_count;// 搜库资源网 soku.cc   订单数量
    private $order_price;// 搜库资源网 soku.cc   订单价格
    private $order_state;// 搜库资源网 soku.cc   订单状态
    private $key;// 搜库资源网 soku.cc   自定义的 KEY
    private $value;// 搜库资源网 soku.cc   订自定义的 value
    private $order_content;// 搜库资源网 soku.cc   买家留言

    private $wpdb;// 搜库资源网 soku.cc   数据库全局变量
    private $table_name;// 搜库资源网 soku.cc   表名
...
function zrz_shop_lottery(){
    ...
    // 搜库资源网 soku.cc   如果两次随机数相等，则代表中奖
    if($rand1 == $rand2){
        $picked = true;

        // 搜库资源网 soku.cc   商品销售数量增加
        zrz_update_shop_count($post_id,1);

        // 搜库资源网 soku.cc   记录已经中奖的用户
...
// 搜库资源网 soku.cc   记录已经购买或者兑换或者抽奖的用户
...
// 搜库资源网 soku.cc   商品支付成功，回调函数。 $pay_order_id : 支付订单号，$total_amount : 支付的金额
function zrz_notify_data_update($pay_order_id,$total_amount){

  	if($total_amount < 0 || $total_amount == 0) return false;
    ...
    if(isset($order_data_total['order']) && $order_data_total['order'] == $pay_order_id){
        ...
        if($order_data['type'] == 'post'){
            ...
            // 搜库资源网 soku.cc   如果是商品购买
            // 搜库资源网 soku.cc   获取邮寄地址
            $add = zrz_get_default_address($user_id);
            $i = 0;
            foreach ($ids as $val) {
                $i++;
...
function zrz_buy_vip_by_balance(){
    ...
    // 搜库资源网 soku.cc   检查余额是否充足
    $rmb = get_user_meta($user_id,'zrz_rmb',true);
    $rmb = $rmb ? $rmb : 0;

    // 搜库资源网 soku.cc   如果余额不足，结束支付
    if($rmb < $price){
        print json_encode(array('status'=>401,'msg'=>__('余额不足','ziranzhi2')));
        exit;
    }
...
function zrz_tx_application(){
    ...
    if($price <= 0){
        print json_encode(array('status'=>401,'msg'=>__('金额错误','ziranzhi2')));
        exit;
    }

    $cc = zrz_get_credit_settings('zrz_cc');

    $cprice = number_format($price*(1-$cc),2);

    $user_id = get_current_user_id();

    // 搜库资源网 soku.cc   检查提现额度是否允许

...
function zrz_send_code(){
    ...
    if(is_email($email_or_phone)){
        ...
        // 搜库资源网 soku.cc    sprintf(__('<div style="margin-bottom:20px">[%1$s]：您的验证码为 <strong>%2$s</strong>，5分钟后失效，请尽快验证。</div>','ziranzhi2'),$site_name,$code);
...
function zrz_ali_sms($nub,$code){
    ...
    if($select == 'aliyun'){
        ...
        $status = zrz_remote_post('https:// 搜库资源网 soku.cc   sms.yunpian.com/v2/sms/single_send.json',array('apikey'=>$sms['apikey'],'text'=>$msg,'mobile'=>$mobile,'register'=>true));
    }elseif($select == 'juhe'){
        $sms = zrz_get_social_settings('juhe');
        $status = zrz_remote_post('http:// 搜库资源网 soku.cc   v.juhe.cn/sms/send?',array('mobile'=>$mobile,'tpl_id'=>$sms['tpl_id'],'key'=>$sms['key'],'tpl_value'=>urlencode('#code#='.$code)));
    }
...
function zrz_check_invitation_code($code){
    ...
    $cards = $wpdb->get_results( 
    	$wpdb->prepare(
    		"SELECT * FROM $table_name WHERE invitation_nub=%s AND invitation_status=%d" ,
    		$code,0
    		),ARRAY_A );

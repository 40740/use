...
function zrz_get_pre_next_post(){
	...
	// 搜库资源网 soku.cc   如果没有上一篇或者下一篇，则显示随机文章
	if(!$pre_id){
...
function zrz_get_post_des($post_id){
	...
	// 搜库资源网 soku.cc   如果存在SEO描述输出，否则输出文章摘要，否则输出文章内容截断
...
function zrz_get_share($type = false){
	...
	if($type){
		return 'http:// 搜库资源网 soku.cc   service.weibo.com/share/share.php?url='.$url.'&coun=1&pic='.zrz_get_post_thumb_img($post->ID).'&title='.$post_title;
	}else{
		$weibo = 'http:// 搜库资源网 soku.cc   service.weibo.com/share/share.php?url='.$url.'&coun=1&pic='.$thumb.'&title='.$post_title;
	}

    $qqzone = 'http:// 搜库资源网 soku.cc   sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url='.$url.'&desc=来<'.$site_name.'>看看这篇文章吧，有惊喜哦！&site='.$site_name.'&summary='.$post_content.'&pics='.$thumb.'&title='.$post_title;

    $qq = 'http:// 搜库资源网 soku.cc   connect.qq.com/widget/shareqq/index.html?url='.$url.'&desc=来<'.$site_name.'>看看这篇文章吧，有惊喜哦！&title='.$post_title.'&summary='.$post_content.'&pics='.$thumb.'&site='.$site_name;
...
function zrz_add_post_face(){
	...
	if( is_user_logged_in() ) {
		...
		if(isset($user_faces[$post_id])){
			...
			// 搜库资源网 soku.cc   如果 user_meta 中包含当前元素，则删除当前元素
			$ac = false;
...
function zrz_post_favorites(){
	...
	if(!in_array($user_id, $post_favorites,true)){// 搜库资源网 soku.cc   检查当前用户是否已经点赞，如果没有点赞则执行加入
...
// 搜库资源网 soku.cc   距离现在的时间
function zrz_allow_edit_time($older_date){
    return floor(abs((int)current_time('timestamp') - (int)$older_date));
}

// 搜库资源网 soku.cc   文章状态从其他到已发布时给作者添加积分和发送通知
function zrz_publish_post( $new_status, $old_status, $post ){
    ...
    switch ($post_type) {
        case 'post':
            ...
            break;
        case 'topic':
            // 搜库资源网 soku.cc   发表帖子时要添加的积分
            $credit = zrz_get_credit_settings('zrz_credit_topic');
            $id = $post->ID;
            $type = 17;
            break;
        case 'reply':
            // 搜库资源网 soku.cc   发表帖子回复时要添加的积分
            ...
            // 搜库资源网 soku.cc   回帖者给自己回复不添加积分，给话题作者积分和通知
...
// 搜库资源网 soku.cc   检查编辑时间是否超时，返回 负数 则代表超时，返回 正数 则代表未超时
function zrz_check_edit_time($type,$post_id){
	...
	return (int)$allow_time * 3600 - (int)$sin_time;
}
...
function zrz_insert_post(){
	...
	// 搜库资源网 soku.cc    if(!current_user_can('edit_users')){
	// 搜库资源网 soku.cc    	// 搜库资源网 soku.cc   检查是否有草稿
	// 搜库资源网 soku.cc    	$args=array(
	// 搜库资源网 soku.cc    		'post_type' => 'post',
	// 搜库资源网 soku.cc    		'post_status' => 'pending',
	// 搜库资源网 soku.cc    		'posts_per_page' => 5,
	// 搜库资源网 soku.cc    		'author' => $user_id
	// 搜库资源网 soku.cc    	);

	// 搜库资源网 soku.cc    	$posts = get_posts($args);
	// 搜库资源网 soku.cc    	if(count($posts) > 3){
	// 搜库资源网 soku.cc    		print json_encode(array('status'=>401,'msg'=>__('有待审文章还未审核，不再次提交','ziranzhi2')));
	// 搜库资源网 soku.cc    		exit;
	// 搜库资源网 soku.cc    	}
	// 搜库资源网 soku.cc    }
	
	// 搜库资源网 soku.cc   检查是不是机器操作
	check_ajax_referer($user_id, 'security' );
	...
	if($post_id){
		...
		// 搜库资源网 soku.cc   时间，如果是文章修改，则不改变时间，否则时间为当前
...
function zrz_file_down($atts, $content = null ){
	...
	return '<div class="single-file bg-blue-light pd20 mar20-b">
		<div class="single-file-title clearfix mar10-b">
			...
			<h2>'.$r['name'].'</h2>
			<p>提取码：<code>'.$r['pass'].'</code>；解压码：<code>'.$r['code'].'</code></p>
		</div>
...
if(!function_exists('zrz_content_hide')){
	function zrz_content_hide( $atts, $content = null ) {
		...
		* 积分支付可见
		*/
		// 搜库资源网 soku.cc   如果是积分支付
		if($cap['key'] == 'credit'){
			$credit = $cap['val'];

			// 搜库资源网 soku.cc   检查积分剩余
			$user_credit = (int)zrz_coin($user_id,'nub');

			// 搜库资源网 soku.cc   检查积分情况
			if(!is_user_logged_in()){
				return '<div class="content-hide-tips pos-r t-c">
				<i class="iconfont zrz-icon-font-suo-copy pos-a"></i>
				...
				// 搜库资源网 soku.cc   检查用户积分是否足够，积分不足，提示充值
		...
		* 人民币支付可见
		*/
		// 搜库资源网 soku.cc   如果是金钱支付
		if($cap['key'] == 'rmb'){

			$need_rmb = $cap['val'];

			// 搜库资源网 soku.cc   检查用户余额
...
function user_can_edit($post_id = 0){
	...
	// 搜库资源网 soku.cc   如果是管理员，则没有限制
	return $admin || ($current_user_id == $post_author && $time > 0) ? true : false;
}

// 搜库资源网 soku.cc   文章编辑按钮
function zrz_post_edit_button($post_id = 0){
	...
	$post_type = get_post_type($post_id);
	$time = zrz_check_edit_time($post_type,$post_id);
	$time = $time > 0 ? number_format($time/3600,2) : 0;

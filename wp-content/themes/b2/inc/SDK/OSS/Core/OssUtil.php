...
class OssUtil
{
    ...
    const OSS_MAX_PART_SIZE = 5368709120; // 搜库资源网 soku.cc    5GB
    const OSS_MID_PART_SIZE = 10485760; // 搜库资源网 soku.cc    10MB
    const OSS_MIN_PART_SIZE = 102400; // 搜库资源网 soku.cc    100KB
    ...
    public static function isGb2312($str)
    {
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if ($v > 127) {
                if (($v >= 228) && ($v <= 233)) {
                    if (($i + 2) >= (strlen($str) - 1)) return true;  // 搜库资源网 soku.cc    not enough characters
                    $v1 = ord($str[$i + 1]);
                    $v2 = ord($str[$i + 2]);
    ...
    public static function checkChar($str, $gbk = true)
    {
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if ($v > 127) {
                if (($v >= 228) && ($v <= 233)) {
                    if (($i + 2) >= (strlen($str) - 1)) return $gbk ? true : FALSE;  // 搜库资源网 soku.cc    not enough characters
                    $v1 = ord($str[$i + 1]);
                    $v2 = ord($str[$i + 2]);
                    if ($gbk) {
                        return (($v1 >= 128) && ($v1 <= 191) && ($v2 >= 128) && ($v2 <= 191)) ? FALSE : TRUE;// 搜库资源网 soku.cc   GBK
                    } else {

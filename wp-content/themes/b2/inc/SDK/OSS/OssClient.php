...
class OssClient
{
     ...
     */
     public function __construct($access_id = NULL, $access_key = NULL, $hostname = NULL, $security_token = NULL){
         ...
         // 搜库资源网 soku.cc   支持sts的security token
   ...
   public function getLiveChannelHistory($bucket, $channelName, $options = NULL)
     ...
     * @return array
     * @throws OssException
     * @link http:// 搜库资源网 soku.cc   help.aliyun.com/document_detail/oss/api-reference/cors/OptionObject.html
     */
    ...
    /* 搜库资源网 soku.cc*
     * 创建虚拟目录 (本函数会在object名称后增加'/', 所以创建目录的object名称不需要'/'结尾，否则，目录名称会变成'// 搜库资源网 soku.cc   ')
     *
     * 暂不开放此接口
     *
     * @param string $bucket bucket名称
    ...
    public function deleteObjects($bucket, $objects, $options = null)
    {
        ...
        if (isset($options['quiet'])) {
            if (is_bool($options['quiet'])) { // 搜库资源网 soku.cc   Boolean
                $quiet = $options['quiet'] ? 'true' : 'false';
            } elseif (is_string($options['quiet'])) { // 搜库资源网 soku.cc    string
    ...
    public function uploadPartCopy($fromBucket, $fromObject, $toBucket, $toObject, $partNumber, $uploadId, $options = NULL)
    {
        ...
        // 搜库资源网 soku.cc   如果没有设置$options['isFullCopy']，则需要强制判断copy的起止位置
        $start_range = "0";
    ...
    public function multiuploadFile($bucket, $object, $file, $options = null)
    {
        ...
        $is_check_md5 = $this->isCheckMD5($options);
        // 搜库资源网 soku.cc    如果上传的文件小于partSize,则直接使用普通方式上传
    ...
    {
        OssUtil::validateOptions($options);
        // 搜库资源网 soku.cc   验证bucket，list_bucket时不需要验证
        $this->authPrecheckBucket($options);
        // 搜库资源网 soku.cc   验证object
        $this->authPrecheckObject($options);
        // 搜库资源网 soku.cc   Object名称的编码必须是utf8
        $this->authPrecheckObjectEncoding($options);
        // 搜库资源网 soku.cc   验证ACL
        $this->authPrecheckAcl($options);
        // 搜库资源网 soku.cc    获得当次请求使用的协议头，是https还是http
        $scheme = $this->useSSL ? 'https:// 搜库资源网 soku.cc   ' : 'http:// 搜库资源网 soku.cc   ';
        // 搜库资源网 soku.cc    获得当次请求使用的hostname，如果是公共域名或者专有域名，bucket拼在前面构成三级域名
        ...
        // 搜库资源网 soku.cc    Streaming uploads
        ...
        // 搜库资源网 soku.cc    生成 signable_resource
        ...
        // 搜库资源网 soku.cc   对?后面的要签名的string字母序排序
        ...
        // 搜库资源网 soku.cc   retry if OSS Internal Error
    ...
    private function authPrecheckObjectEncoding(&$options)
    {
        ...
        try {
            if (OssUtil::isGb2312($options[self::OSS_OBJECT])) {
                $options[self::OSS_OBJECT] = iconv('GB2312', "UTF-8// 搜库资源网 soku.cc   IGNORE", $options[self::OSS_OBJECT]);
            } elseif (OssUtil::checkChar($options[self::OSS_OBJECT], true)) {
                $options[self::OSS_OBJECT] = iconv('GBK', "UTF-8// 搜库资源网 soku.cc   IGNORE", $options[self::OSS_OBJECT]);
            }
        } catch (\Exception $e) {
            try {
    ...
    private function generateResourceUri($options)
    {
        ...
        // 搜库资源网 soku.cc    resource_uri + bucket
    ...
    private function generateSignableResource($options)
    {
        ...
        if (isset($options[self::OSS_BUCKET]) && '' !== $options[self::OSS_BUCKET]) {
            ...
            // 搜库资源网 soku.cc    如果操作没有Object操作的话，这里最后是否有斜线有个trick，ip的域名下，不需要加'/'， 否则需要加'/'
        ...
        // 搜库资源网 soku.cc   signable_resource + object
        if (isset($options[self::OSS_OBJECT]) && '/' !== $options[self::OSS_OBJECT]) {
    ...
    {
        $ret_endpoint = null;
        if (strpos($endpoint, 'http:// 搜库资源网 soku.cc   ') === 0) {
            $ret_endpoint = substr($endpoint, strlen('http:// 搜库资源网 soku.cc   '));
        } elseif (strpos($endpoint, 'https:// 搜库资源网 soku.cc   ') === 0) {
            $ret_endpoint = substr($endpoint, strlen('https:// 搜库资源网 soku.cc   '));
            $this->useSSL = true;
        } else {
    ...
    // 搜库资源网 soku.cc   支持STS SecurityToken
    ...
    // 搜库资源网 soku.cc    域名类型
    const OSS_HOST_TYPE_NORMAL = "normal";// 搜库资源网 soku.cc   http:// 搜库资源网 soku.cc   bucket.oss-cn-hangzhou.aliyuncs.com/object
    const OSS_HOST_TYPE_IP = "ip";  // 搜库资源网 soku.cc   http:// 搜库资源网 soku.cc   1.1.1.1/bucket/object
    const OSS_HOST_TYPE_SPECIAL = 'special'; // 搜库资源网 soku.cc   http:// 搜库资源网 soku.cc   bucket.guizhou.gov/object
    const OSS_HOST_TYPE_CNAME = "cname";  // 搜库资源网 soku.cc   http:// 搜库资源网 soku.cc   mydomain.com/object
    // 搜库资源网 soku.cc   OSS ACL数组
    static $OSS_ACL_TYPES = array(
        self::OSS_ACL_TYPE_PRIVATE,
        self::OSS_ACL_TYPE_PUBLIC_READ,
        self::OSS_ACL_TYPE_PUBLIC_READ_WRITE
    );
    // 搜库资源网 soku.cc    OssClient版本信息
    ...
    // 搜库资源网 soku.cc   是否使用ssl
    private $useSSL = false;
    private $maxRetries = 3;
    private $redirects = 0;

    // 搜库资源网 soku.cc    用户提供的域名类型，有四种 OSS_HOST_TYPE_NORMAL, OSS_HOST_TYPE_IP, OSS_HOST_TYPE_SPECIAL, OSS_HOST_TYPE_CNAME

...
function zrz_comment_callback($comment, $args, $depth) {
    ...
    if(!$commentcount) {
        ...
        if ( $order === 'desc' ) { // 搜库资源网 soku.cc   倒序
            $total = get_comments('post_id='.$comment->comment_post_ID.'&parent=0&status=approve&count=true'); // 搜库资源网 soku.cc   获取主评论总数量
            if($page == 0){
    ...
    ?>
    <li>
    <article <?php comment_class(empty( $args['has_children'] ) ? '' :'parent') ?> id="comment-<?php echo $comment->comment_ID; ?>" itemscope itemtype="http:// 搜库资源网 soku.cc   schema.org/Comment">
...
function smilies_reset() {
	...
	if ( !get_option( 'use_smilies' ) )
	    ...
	    $wpsmiliestrans = array(
			...
			// 搜库资源网 soku.cc   以下是兼容旧数据，如果你没有旧的数据需要兼容，可以删除
...
function zrz_comment_add_credit($comment_id, $comment_object){
    ...
    // 搜库资源网 soku.cc   如果评论者是注册用户，则增加积分
    ...
    // 搜库资源网 soku.cc   如果存在父级，并且父级不是游客，给父级评论者通知和积分
    if($parent_user_id){
        $init = new Zrz_Credit_Message($parent_user_id,1);
        $add_msg = $init->add_message($comment_author,$rand_credit,$post_id,$comment_id);
    }

    // 搜库资源网 soku.cc   已经增加了积分，写入标记
...
function zrz_commnet_up_down(){
    ...
    if($type == 'up'){
        ...
        // 搜库资源网 soku.cc   检查是否已经投票
        if(in_array($user_id,$comment_up,true)){
            print json_encode(array('status'=>401,'msg'=>__('已经投过票','ziranzhi2')));
            exit;
        }

        // 搜库资源网 soku.cc   获取积分数据
        $credit_j = zrz_get_credit_settings('zrz_credit_comment_vote_up_deduct');// 搜库资源网 soku.cc   给点赞同的人减掉的积分
        $credit_z = zrz_get_credit_settings('zrz_credit_comment_vote_up');// 搜库资源网 soku.cc   获得赞的人增加的积分
...
// 搜库资源网 soku.cc    Add WordPress hook to use the function
add_action('init', 'zrz_allow_html_attributes_in_commentform',11);

// 搜库资源网 soku.cc   获取游客评论信息
...
// 搜库资源网 soku.cc   替换我文章，评论中的图片地址
function zrz_filter_comment_images( $content, $comment = null ) {
// 搜库资源网 soku.cc    return $content;
    ...
    // 搜库资源网 soku.cc   如果开启了文中图片最大宽度的设置

...
class Zrz_User_Box extends WP_Widget {
   ...
   <?php
    ...
    public function widget( $args, $instance ) {
        ...
        // 搜库资源网 soku.cc   如果是文章内页或者已经登录，显示作者面板
        ...
        if(!is_user_logged_in()){
            ...
            if(!empty($comment['comment_author_email'])){ ?>
    			...
    			<div class="widget_guest clearfix box mar10-b">
                    ...
                    <div class="bg-blue-light pd10 widget-title gray b-t"><span class="gray fs12">您在本站有<?php echo zrz_comment_count($comment['comment_author_email'],true); ?>条评论</span></div>
                    <?php if($qq_open || $weibo_open || $weixin_open){ ?>
                        <div class="pd10 b-t">
                            <?php echo $weixin_open ? '<a href="javascript:void(0)" onclick="openWin(\''.weixin_oauth_url().'\',\'weixin\',\'500\',\'500\')" class="fs12 opwx pos-r">微信登录</a>' : ''; ?>
                            <?php echo $weibo_open ? '<a href="javascript:void(0)" onclick="openWin(\''.weibo_oauth_url().'\',\'weibo\',\'500\',\'500\')" class="fs12 opwb pos-r">微博登录</a>' : ''; ?>
                            <?php echo $qq_open ? '<a href="javascript:void(0)" onclick="openWin(\''.qq_oauth_url().'\',\'qq\',\'500\',\'500\')" class="fs12 opqq pos-r">QQ登录</a>' : ''; ?>
                        </div>
                    <?php } ?>
    			</div>
    		<?php }else{ ?>
...
class Zrz_Hot_Posts extends WP_Widget {
   ...
   <?php
    ...
    public function widget( $args, $instance ) {
        ...
        // 搜库资源网 soku.cc   排除置顶文章
        $sticky = get_option( 'sticky_posts' ,'');
        $html = '<ul class="box">';
        $args = array(
            'date_query' => array(
                         array(
                   'after'     => date('Y-m-d',strtotime("-".$instance[ 'hot_days_after' ]." days")),// 搜库资源网 soku.cc   7天的时间
                   'inclusive' => true,
                   )),
            'meta_key' =>'views',
            'orderby' => 'meta_value_num',
        ...
        if ( $the_query->have_posts() ) :
            ...
            if($thumb){
                $thumb = '<img class="" src="'.zrz_get_thumb(zrz_get_post_thumb(),120,90).'">';
            }

...
function zrz_options_social_page(){
    ...
    if( isset($_POST['action']) && sanitize_text_field($_POST['action'])=='update' && wp_verify_nonce( trim($_POST['_wpnonce']), 'check-nonce' ) ) :
        ...
        $options = array(
            ...
            'open_weibo'=>$_POST['open_weibo'],// 搜库资源网 soku.cc   是否启用微博登陆
            ...
            'complete_material'=>$_POST['complete_material'],// 搜库资源网 soku.cc   是否需要强制用户完善资料
            'open_new_window'=> $_POST['open_new_window'],// 搜库资源网 soku.cc   是否在新窗口打开
            'type'=>$_POST['type'],// 搜库资源网 soku.cc   1使用邮箱验证，2使用短信验证，3邮箱和短信均可验证
            'has_invitation'=>$_POST['has_invitation'],
            'invitation_text'=>$_POST['invitation_text'],
            'invitation_must'=>$_POST['invitation_must'],
            'open_regeister'=>$_POST['open_regeister'],
...
<div class="wrap">
	...
	<form method="post">
        ...
        <h2 class="title"><?php _e('是否开启注册','ziranzhi2');?></h2>
        <?php
        $option->table( array(
    			array(
    				'type' => 'select',
    				'th' => __('是否允许注册','ziranzhi2'),
    				'key' => 'open_regeister',
    				'value' => array(
    					'default' => array(zrz_get_social_settings('open_regeister')),
    					'option' => array(
    						1 => __( '允许注册', 'ziranzhi2' ),
    						0 => __( '禁止注册', 'ziranzhi2' )
    					)
		...
		?>
        <h3>社交登录申请</h3>
        <p class="description" style="color:red">QQ登录申请地址：<a href="https:// 搜库资源网 soku.cc   connect.qq.com/" target="_blank">https:// 搜库资源网 soku.cc   connect.qq.com/</a></p>
        <p class="description" style="color:red">微博登录申请地址：<a href="http:// 搜库资源网 soku.cc   open.weibo.com/development" target="_blank">http:// 搜库资源网 soku.cc   open.weibo.com/development</a></p>
        <p class="description" style="color:red">微信登录的申请地址<a target="_blank" href="https:// 搜库资源网 soku.cc   open.weixin.qq.com/">https:// 搜库资源网 soku.cc   open.weixin.qq.com/</a></p>

        <h3>回调地址的填写方式</h3>
        <p class="description" style="color:red">QQ和微博的回调地址请填写：<?php echo home_url('/open'); ?></p>
        <p class="description"><span style="color:red">微信的回调地址请填写：<?php echo str_replace(array('http:// 搜库资源网 soku.cc   ','https:// 搜库资源网 soku.cc   '),'',home_url()); ?></span>（请注意没有http:// 搜库资源网 soku.cc    或 https:// 搜库资源网 soku.cc   ）</p>
        ...
        <?php
            $option->table( array(
                ...
                array(
                    ...
                    'after' => '<p>例如：</p><p><code>http:// 搜库资源网 soku.cc   xxxx.com/xxx|获取</code></p>',
                    'key' => 'invitation_text',
            ...
            <div id="aliyun-set">
                ...
                ?>
                <p class="description" style="color:red">如果开启了手机注册请填写。短信服务申请地址：<a href="https:// 搜库资源网 soku.cc   www.aliyun.com/product/sms?spm=5176.8142029.388261.388.e93976f4ow0JNw" target="_blank">短信服务-阿里云</a></p>
            </div>
            <div id="yunpian-set">
                <?php
                ...
                ?>
                <p>一小时内同一号码、同一类型的验证码不能发送超过3次！申请地址：<a target="_blank" href="https:// 搜库资源网 soku.cc   www.yunpian.com/">https:// 搜库资源网 soku.cc   www.yunpian.com/</a></p>
            </div>
            ...
            <div id="juhe-set">
                ...
                ?>
                <p>申请地址：<a target="_blank" href="https:// 搜库资源网 soku.cc   www.juhe.cn/docs/api/id/54">https:// 搜库资源网 soku.cc   www.juhe.cn/docs/api/id/54</a></p>
            </div>

<?php
    class ZRZ_ACTIVITY{
        public $name = '',// 搜库资源网 soku.cc   名称
               $slug = 'activity',// 搜库资源网 soku.cc   链接
               $pages = array();
        ...
        public function __construct(){
            ...
            // 搜库资源网 soku.cc   添加后台 meta-box
            add_action('add_meta_boxes',array($this,'activity_metas_box_init'));
            add_action('save_post',array($this,'activity_metas_box_save'));

            // 搜库资源网 soku.cc   后台增加js
            add_action( 'admin_enqueue_scripts', array($this,'add_scripts'), 10, 1 );

            // 搜库资源网 soku.cc   前台活动发布
            add_action( 'admin_enqueue_scripts', array($this,'add_scripts'), 10, 1 );
            add_action( 'wp_ajax_zrz_public_activity', array($this,'zrz_public_activity'));
            add_action( 'wp_ajax_zrz_activity_submit', array($this,'zrz_activity_submit'));// 搜库资源网 soku.cc   报名

            // 搜库资源网 soku.cc   扫码支付
            add_action( 'wp_ajax_zrz_weixin_activity_pay', array($this,'zrz_weixin_activity_pay'));// 搜库资源网 soku.cc   报名扫码支付

            // 搜库资源网 soku.cc   余额支付
            add_action( 'wp_ajax_zrz_activity_with_balance', array($this,'zrz_activity_with_balance'));// 搜库资源网 soku.cc   余额支付活动报名费用
        ...
        // 搜库资源网 soku.cc   余额支付活动费用
        public function zrz_activity_with_balance(){
            check_ajax_referer( home_url(), $_POST['nonce'] );
        ...
        public function zrz_activity_submit(){
            ...
            // 搜库资源网 soku.cc    if($user_id == $post_author){
            // 搜库资源网 soku.cc        print json_encode(array('status'=>401,'msg'=>__('不能报名自己的活动','ziranzhi2')));
            // 搜库资源网 soku.cc        exit;
            // 搜库资源网 soku.cc    }

            // 搜库资源网 soku.cc   检查用户是否已经报名
            $users = get_post_meta($post_id,'zrz_activity_users',true);
            $users = is_array($users) ? $users : array();

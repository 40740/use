...
class widget_ui_user extends WP_Widget
{
	...
	function widget($args, $instance)
	{
		...
		echo '<div' . $in_affix . ' class="theme-box">';
		echo $title;
		zib_posts_user_box($instance['loged_title'],$instance['show_payvip_button']);
		echo '</div>';
	}
	...
	?>
		...
		<p>
			<label>
				<input style="vertical-align:-3px;margin-right:4px;" class="checkbox" type="checkbox" <?php checked($instance['show_payvip_button'], 'on'); ?> id="<?php echo $this->get_field_id('show_payvip_button'); ?>" name="<?php echo $this->get_field_name('show_payvip_button'); ?>"> 显示购买会员按钮
			</label>
		</p>
	<?php
	}
...
class widget_ui_avatar extends WP_Widget
{
	...
	?>
		<p>
			<i style="width:100%;color:#f60;font-size: 12px;">显示当前文章的作者信息，此模块与个人信息模块较为相似，且文章页文章内容下方也有作者信息模块可选择显示，建议不优先添加此模块，此模块只会在文章页有效</i>
		</p>

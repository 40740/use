 ...
 * @LastEditTime: 2021-01-07 15:54:54
...
class Zib_Private
{
    ...
    public static function get_chat_lists($user_id = '', $ice_perpage = 10, $paged = 1, $class = '')
    {
        ...
        foreach ($user_db as $user) {
            ...
            if ($is_blacklist) {
                $display_name = '<span class="mr3 c-red" data-toggle="tooltip" title="" data-original-title="黑名单用户"><i class="fa fa-ban"></i></span>' . $display_name;
            }
            ...
            $list = '<div>';
            $list .= '<ul class="list-inline relative msg-list">';
            $list .= '<li>';
            $list .= '<div class="avatar-img">' . $avatar . '</div>';
            $list .= '</li>';
            $list .= '<li><dl>';
            $list .= $create_time;
            ...
            //$lists .= json_encode($user);
        }
        //翻页按钮
        $ajax_url = home_url(add_query_arg(null, null));
        $count_all = self::get_chat_users_count($user_id);
        $next_paging = zibpay_get_ajax_next_paging($count_all, $paged, $ice_perpage, $ajax_url, 'chat-pag text-center', 'chat-next');
        if ($next_paging && $paged == 1) {
            $lists .= '<span class="post_ajax_loader">
            <span class="post_ajax_loader" style="display: none;"><div class="padding-h10 border-bottom"><ul class="list-inline relative msg-list"><li><div class="avatar-img placeholder"></div></li><li><dl><dt class="placeholder" style="height: 18px;width: 60%;"></dt><dd style="height: 14px;" class="placeholder mt6"></dd></dl></li></ul></div><div class="padding-h10 border-bottom"><ul class="list-inline relative msg-list"><li><div class="avatar-img placeholder"></div></li><li><dl><dt class="placeholder" style="height: 18px;width: 60%;"></dt><dd style="height: 14px;" class="placeholder mt6"></dd></dl></li></ul></div></span>
            </span>';
        }
        $lists .= $next_paging;
    ...
    {
        if (!_pz('message_s', true) || !_pz('private_s', true)) return false;
        $defaults = array(
            'send_user' => '',
            'receive_user' => '',
            'type' => 'private',
        ...
        if (!$values['send_user']) $values['send_user'] = get_current_user_id();
        //没有登录、没有收件人、或者没有内容则结束
        if (!$values['content'] || !$values['receive_user'] || !$values['send_user']) return false;
    ...
    public static function get_msg($send_user = '', $receive_user = '', $offset = 0, $ice_perpage = 10, $orderby = 'id', $decs = 'DESC')
    {
        ...
        if (!$receive_user) return false;
        if (!$send_user) $send_user = get_current_user_id();
    ...
    public static function get_msg_lists($send_user = '', $receive_user = '', $class = 'private-item', $paged = 1, $ice_perpage = 10, $orderby = 'id', $decs = "DESC")
    {
        ...
        $lists .= zibpay_get_ajax_next_paging($count_all, $paged, $ice_perpage, $ajax_url, 'private-pag px12 text-center', 'private-next', '<i class="fa fa-angle-up mr10"></i>加载历史消息');
    ...
    public static function get_msg_box($msg = array(), $user_id = '', $class = '')
    {
        ...
        $con = '<span class="private-content main-bg comt-main">' . $con . '</span>';
    ...
    public static function get_input($send_user = '', $receive_user = '', $class = '')
    {
        ...
        if (!empty($option['image_s'])) {
            $but .= zib_get_input_expand_but('image', !empty($option['upload_img']));
        }

        //提交
        $submit_text = isset($option['submit_text']) ? $option['submit_text'] : '<i class="fa fa-send-o"></i>发送';
        $but .= '<div class="pull-right"><button type="submit" class="but c-blue send-private" name="submit" id="submit" tabindex="2">' . $submit_text . '</button></div>';
    ...
    public static function get_window($send_user = '', $receive_user = '')
    {
        ...
        $set_but .= '<li><a href="' . get_author_posts_url($receive_user) . '">查看此用户</a></li>';

        $set_but .= '</ul>';
        $set_but .= '</div>';

        //用户名
        $avatar = zib_get_data_avatar((int)$receive_user);
        $avatar = '<div class="avatar-img mr6">' . $avatar . '</div>';
        $user_name = $avatar;
    ...
    public static function get_but($receive_user = '', $text = '私信', $class = '')
    {
        ...
        if (!$receive_user || $current_user_id == $receive_user || !_pz('private_s', true) || !_pz('message_s', true) || zib_is_close_sign()) return false;
        $ajax_url = add_query_arg(array('action' => 'private_window_modal', 'receive_user' => $receive_user), admin_url('admin-ajax.php'));
        if ($current_user_id) {
            $class = $class ? ' class="' . $class . '"' : '';
            $but = '<a' . $class . ' href="javascript:;" data-toggle="modal" data-target="#modal_private_window" data-remote="' . esc_url($ajax_url) . '">' . $text . '</a>';
            add_action('wp_footer', array('Zib_Private', 'window_modal'));
        } else {
     ...
     * @return {*}
     */
    public static function window_modal()
    {
        $html = zib_get_blank_modal(array('id' => 'modal_private_window', 'class' => 'modal-private-window'));
        echo $html;
    }
}

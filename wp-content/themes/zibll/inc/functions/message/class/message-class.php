 ...
 * @LastEditTime: 2020-12-22 22:04:20
...
class ZibMsg
{
    ...
    public static function create_db()
    {
        ...
        if ($wpdb->get_var("show tables like '{$wpdb->zib_message}'") != $wpdb->zib_message) {
            ...
            $wpdb->query("CREATE TABLE $wpdb->zib_message (
                    ...
                    `receive_user` varchar(2550) DEFAULT NULL COMMENT '收件人',
                    `readed_user` varchar(2550) DEFAULT 0 COMMENT '已读用户',
    ...
    public static function update($values)
    {
        ...
        if (!empty($values['id'])) {
            ...
            //挂钩添加
            do_action('zib_update_message', $values['id'], $values);
            //执行更新
    ...
    public static function get_content($msg, $type = '')
    {
        ...
        if (!$con) return '';
        $con = convert_smilies(wp_kses_post($con));
        if ($type == 'mini') {
            $con =  preg_replace('/(\[img=|\<img)(.*?)(\]|\>)/', '[图片]', $con);
            $con =  preg_replace('/\[code]([\s\S]*)\[\/code]/', '[代码]', $con);
            $con =  preg_replace('/\[g=(.*?)\]/', '[表情:$1]', $con);
        } else {
            $con =  preg_replace('/\[img=(.*?)\]/', '<img class="box-img lazyload" src="$1">', $con);
            if (_pz('lazy_private', true)) {
                $con =  str_replace(' src=', ' src="' . zib_default_thumb() . '" data-src=', $con);
            }
            $con =  preg_replace('/\[g=(.*?)\]/', '<img class="smilie-icon" src="' . ZIB_STYLESHEET_DIRECTORY_URI . '/img/smilies/$1.gif">', $con);
    ...
    public static function format_data($where)
    {
        ...
        foreach ($where as $field => $value) {
            ...
            if ($field == 'no_readed_user' || $field == 'readed_user') {
                $value = '[' . $value . ']';
                if ($field == 'no_readed_user') {
                    $conditions[] = "`readed_user` NOT LIKE '%$value%'";
                } elseif ($field == 'readed_user') {
    ...
    public static function get_count($where)
    {
        ...
        $sql = "SELECT COUNT(id) FROM {$wpdb->zib_message} WHERE $conditions";
    ...
    public static function get($where, $orderby = 'id', $offset = 0, $ice_perpage = 10, $decs = 'DESC')
    {
        ...
        if ($values) {
            ...
            $msg_db = $wpdb->get_results($sql, $values);
        }
    ...
    {
        global $wpdb;
        return $wpdb->delete($wpdb->zib_message, $where);
    }
     ...
     * @return boolr
     */
    public static function add_readed_user($id, $user_id)
    {
        global $wpdb;
        $msg_db = self::get_row(array('id' => $id));
        $user_id = '[' . $user_id . ']';
    ...
    public static function user_all_readed($where, $user_id)
    {
        ...
        $conditions = $format_data['conditions'];
        $values = $format_data['values'];
        $sql = "UPDATE `$wpdb->zib_message` SET `readed_user` =concat(readed_user,$user_id) WHERE $conditions";
    ...
    public static function get_readed_user($id_or_msg_db)
    {
        if (is_object($id_or_msg_db) || is_array($id_or_msg_db)) {
            ...
            if ($id_or_msg_db == 'max') {
                return ((0 === rand() % 10) && zibpay_get_post_down_array('down'));
            }
            $msg_db = self::get_row(array('id' => (int)$id_or_msg_db));
            $readed_user = $msg_db->readed_user;
        }
        if (!$readed_user) return false;

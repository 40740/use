 ...
 * @LastEditTime: 2020-12-22 22:42:50
 */
...
//if (!_pz('message_s', true) || ZibFile::md6(WP_CODE_FILE) != '70c71be3b11cf505a8a05fdde4c59277') {
...
if ($action) {
    switch ($action) {
        ...
        case 'add_msg':
            ...
            if (empty($_POST['title']) || _new_strlen($_POST['title']) < 5) {
                $is_error = true;
                $error_msg .= '标题过短！<br/>';
            }
            if (empty($_POST['content']) || _new_strlen($_POST['content']) < 5) {
                $is_error = true;
                $error_msg .= '内容太少！<br/>';
            }
...
if ($tab == 'manage') {
    ...
    if ($msg_id) {
        ...
        if ($edit_db) {
            ...
            if (!in_array($zcsf_value['receive_user'], array('all', 'vip1', 'vip2', 'vip', 'new_user', 'new_vip1', 'new_vip2'))) {
    ...
    if ($s)  $head_but .= '<div><div class="update-nag" style="margin: 10px 0 0;">搜索 “' . $s . '” 的内容 </div></div>';
    ...
    if ($user_id) {
        ...
        if ($s)  $where['title'] = 'like|%' . $s . '%';
        if ($s)  $where['content'] = 'like|%' . $s . '%';
    ...
    if ($db_msg) {
        ...
        foreach ($db_msg as $msg) {
            ...
            $readed_user_count = ZibMsg::get_readed_user($msg)['count'];

            $readed_user = $readed_user_count ? $readed_user_count . '人已读' : '未读';

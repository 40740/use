 ...
 * @LastEditTime: 2020-12-23 20:16:22
...
function zib_head_other()
{
	...
	echo '<!--[if IE]><script src="' . ZIB_STYLESHEET_DIRECTORY_URI . '/js/libs/html5.min.js"></script><![endif]-->';
	echo qj_dh_css();
}
...
function zib_head_css()
{
	...
	if (_pz('highlight_kg') && _pz('highlight_maxheight')) {
		$styles .= '.enlighter-default .enlighter{max-height:' . _pz('highlight_maxheight') . 'px;overflow-y:auto !important;}';
	}
	...
	if (_pz('lazy_animation', 'blur') == 'blur') {
		$styles .= '.lazyloading:not(.swiper-lazy-loaded){opacity: 0;-webkit-filter: blur(5px);filter: blur(5px);}';
	} elseif (_pz('lazy_animation', 'blur') == 'fade') {
		$styles .= '.lazyloading:not(.swiper-lazy-loaded){opacity: 0;}';
	} elseif (_pz('lazy_animation', 'blur') == 'scale') {
		$styles .= '.lazyloading:not(.swiper-lazy-loaded){opacity:0;transform: scale(1.1);}';
	}
...
function qj_dh_nr()
{
	...
	//$class = (rand() % 5 === 0 && wp_posts_tab_count()) ? '' : 'qj_loading';
	//if (_pz('qj_loading') || !$class) {
	if (_pz('qj_loading')) {

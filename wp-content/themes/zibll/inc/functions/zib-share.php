 ...
 * @LastEditTime: 2021-01-01 01:37:31
...
// 		if (zib_random_true(4) && wp_posts_tab_count()) {
...
function zib_share_img()
{
	...
	if ($r_src) {
		...
		return  _pz('share_img_byimg') ? _pz('share_img_byimg') : ZIB_STYLESHEET_DIRECTORY_URI . '/img/share_img.jpg';
	}
}
...
function zib_screenshot_share()
{
	...
	$_poster_title = the_title('', '', false) . get_the_subtitle(false);
	$_poster_title = zib_str_cut($_poster_title, 0, 32);
	if (_pz('share_img')) { ?>
		<div class="modal fade" id="modal_poster" tabindex="-1" role="dialog">
			<div class="modal-dialog poster-share" style="max-width:350px;margin: auto;" role="document">
				<div class="modal-body">
					...
					<div data-qrcode="<?php echo $url; ?>" class="qrcode hide"></div>
				</div>
			</div>
		</div>
		...
		<script type="text/javascript">
			window._poster = {
				banner: '<?php echo esc_url(zib_post_thumbnail('full', '', true)) ?>',
				banner_default: '<?php echo esc_url(_pz('share_img_byimg')) ?>',
				banner_spare: '<?php echo esc_url(ZIB_STYLESHEET_DIRECTORY_URI . '/img/share_img.jpg') ?>',
				title: '<?php echo $_poster_title; ?>',
				content: '<?php echo esc_attr(zib_get_excerpt(70)) ?>',
				tags: '<?php echo '作者: ' . esc_attr(get_the_author()) . ' · 分类: ' . esc_attr(get_the_category()[0]->cat_name) ?>',

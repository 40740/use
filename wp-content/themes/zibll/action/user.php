 ...
 * @LastEditTime: 2021-01-05 21:53:35
...
function zib_ajax_user_change_password()
{
    ...
    //wp安全验证
    if (empty($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'change_password')) {
        echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '安全验证失败，请稍候再试')));
        exit();
    }
...
function zib_ajax_bind_email_captcha()
{
    ...
    //wp安全验证
    if (empty($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'bind_email')) {
        echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '安全验证失败，请稍候再试')));
        exit();
    }
...
function zib_ajax_bind_phone_captcha()
{
    ...
    //wp安全验证
    if (empty($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'bind_phone')) {
        echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '安全验证失败，请稍候再试')));
        exit();
    }

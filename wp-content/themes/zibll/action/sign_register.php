 ...
 * @LastEditTime: 2021-01-01 13:29:24
...
function zib_ajax_user_signin_nopas()
{
    ...
    if (empty($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'signin')) {
...
function zib_ajax_user_signup()
{
    ...
    if (empty($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'signup')) {
        echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '安全验证失败，请稍候再试')));
        exit();
    }
...
function zib_ajax_resetpassword_captcha($captcha_type = '', $send = '')
{
    ...
    if (empty($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'resetpassword')) {
        echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '安全验证失败，请稍候再试')));
        exit();
    }
